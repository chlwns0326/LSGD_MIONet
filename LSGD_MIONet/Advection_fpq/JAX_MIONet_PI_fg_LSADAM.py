...
folder = 'Models/'
keyword = 'Advection_fpq_PI' # 
model_desc = 'L3W100'
datafolder = '../data/'

# Hyperparameters
LS_num = 1 # Adam = 0, LS+Adam > 0
weights_init = [1e-1,1,1e-6]  # weights for physics, data, regularization term, resp. 
...
u_g_in_foo = jnp.zeros((batch_size[1],2*Nx+1)) # Long 1D input
...
j = 1 # jump/stride
uin_f_train = jnp.asarray(data['input_f_train'].astype('float64'))[:N_train[0],:][:,::j]
uin_f_val = jnp.asarray(data['input_f_val'].astype('float64'))[:N_val,:][:,::j]
