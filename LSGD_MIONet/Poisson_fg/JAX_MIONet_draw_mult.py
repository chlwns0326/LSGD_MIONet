...
def draw_sample(xy_in,uin_f,uin_g,uout,uout_pred,inds,keyword,savedir):
    ...
    for ind in inds:
        ...
        uin_f_ind,uin_g_ind = uin_f[ind,:,:,0], uin_g[ind,:]
        
        ax = fig.add_subplot(2,3,1,projection='3d')
        im = ax.plot_surface(x_in, y_in, uin_f_ind.T, cmap=cm.coolwarm, linewidth=0, antialiased=False)
        plt.colorbar(im,ax=ax)
        ax.set_title(f'Input function F')
        
        ax = fig.add_subplot(2,3,2)
        im = ax.plot(jnp.linspace(0,4,jnp.shape(uin_g_ind)[0]), uin_g_ind) # Pois BC
...
for keyword,model_desc,phy,lamb,etime in zip(keywords,model_descs,phys,lambdas,etimes):
    # Hyperparameters
    weights_init = [1,phy,lamb] # weights for data, regularization term, resp. 
  
    # Initialize & Data load
    if keyword == 'Poisson_fg':
        u_f_in_foo = jnp.zeros((1,Nx+1,Ny+1,1)) # 2D input
        u_g_in_foo = jnp.zeros((1,4*Nx+1)) # 1D input
        dataname = 'Poisson_fg_scale_data'
        model_settings = [([[16,(3,3),(2,2),'VALID'],[32,(2,2),(2,2),'VALID'],[64,(2,2),(2,2),'VALID']],
                           [150]*1,silu,1,False),([150]*2,silu,1,False),([150]*3,silu,1,False)]
        branch_model_1 = CNN_MLP(*model_settings[0])
    ...
    j = 1 # jump/stride
    uin_f_train = jnp.asarray(data['input_f_train'].astype('float64'))[:N_train,:,:,None][:,::j,:,:][:,:,::j,:]
    uin_f_val = jnp.asarray(data['input_f_val'].astype('float64'))[:N_test,:,:,None][:,::j,:,:][:,:,::j,:]
    uin_g_train = jnp.asarray(data['input_g_train'].astype('float64'))[:N_train,:][:,::j]
    uin_g_val = jnp.asarray(data['input_g_val'].astype('float64'))[:N_test,:][:,::j]
    ...
    for seed in seeds:
        for ALS_num in ALS_nums:
            phy_lambda = '{:.0e}'.format(weights_init[1])
            # Adam vs LS+Adam
            if ALS_num > 0: # LS+Adam
                ...
                model_dir = folder + keyword + '_PI_ALSAdam_L' + str(ALS_num) + 'R' + str(adam_num) + '_' + \
                    str(delay_init_pre) + '_HHH_SSS_bat' + str(batch_size) + 'SQto' + str(batch_new) + 'SQ_' + \
                    model_desc + '_seed_' + seed + '_phy_' + phy_lambda + '_regul_' + reg_lambda
            else: # Adam
                batch_size, batch_new = 100, 100 # predetermined adam batch 
                model_dir = folder + keyword + '_PI_Adam_NoLS_HHH_SSS_bat' + str(batch_size) + 'SQ_' + \
