...
def train(params, optimizer, seed, delay_init,
        ...
        xy, 
        weights_init, 
        batch_size, batch_new, adam_num, LS_num, LS_frac,
        loss_logs, model_dir,
        nIter, disp_count):
    ...
    uout = uout_train.diagonal(0,0,1).transpose((2,0,1))[perm_train[:forward_train_batch],:,:]
    ...
    for wu in tqdm(range(nIter)):
        ...
        # decay_target = 1e-6 # b
        # decay_start = 50 # Start point of decay, constant before decay_start WU
        # decay_end = 500 # End point of decay, stay constant afterward
        ...
        if wu == 0: # No Adam, init state
            # only forward
            losses = loss_comps(params, uin_f, uin_g, uout, xy, weights)
            ...
            for ad_repeat in range(adam_num):
                ...
                for inum, (find, gind) in enumerate(batch_blocks):
                    ...
                    uout_batch = jnp.reshape(uout_train[find,:,:,:][:,gind,:,:],
                                (uin_f_batch.shape[0]*uin_g_batch.shape[0],uout_train.shape[2],uout_train.shape[3]))
                    params, opt_state = step_GD(params, optimizer, opt_state, uin_f_batch, uin_g_batch, uout_batch, xy, weights)
                    params['last1'].block_until_ready() ### Foo code 
                    part_time_2 = part_time_2 + (time.perf_counter()-timer_2)
                    
                    # Fractional LS steps
                    if LS_frac > 1:
                        if wu > delay_init and inum+1 < num_iters and (inum+1) % (num_iters//LS_frac) == 0:
                            for repLS in range(LS_num): 
                                params['last1'].block_until_ready() ### Foo code 
                                timer_3 = time.perf_counter()
                                params = step_LS_alt_order(params, uin_f_train, uin_g_train, uout_train, xy, weights, order = fg_order) # 0 okey[0]
                                params['last1'].block_until_ready() ### Foo code 
        ...
        # Validation
        losses_val = loss_comps(params, uin_f_val, uin_g_val, uout_val, xy, weights)
