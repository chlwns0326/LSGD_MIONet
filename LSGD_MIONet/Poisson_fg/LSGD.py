...
def construct_LS_PI(params, uin_f, uin_g, xy_phys, xy_data):
    ...
    Fphys = uin_f[:,1:-1,1:-1,0].transpose(0,2,1).reshape((jnp.shape(uin_f)[0],-1)) # P1 by Q
    Tdata = trunk_model.apply(params['trunk'], xy_data) # Qb by I ##
    Fdata = uin_g[:,:-1] # P2 by Qb # zero dirichlet
...
def solve_LS_C2_PI(params, uin_f, uin_g, xy_phys, xy_data, weights):
    ...
    Q1, d1, Q1T = jnp.linalg.svd(BTB + eps*jnp.eye(jnp.shape(B2)[1]), hermitian=True)
