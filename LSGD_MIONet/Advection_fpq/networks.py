...
def residual_net(params, uf, ug, xy):
    ...
    ftilde = jnp.kron(uf[:,1:,None].transpose(0,2,1),jnp.ones((jnp.shape(ug)[0],jnp.shape(uf)[1]-1,1))) # Make P1P2 by Q=(Qx by Qt)
...
def residual_net_paired(params, uf, ug, xy):
    ...
    ftilde = jnp.kron(uf[:,1:,None].transpose(0,2,1),jnp.ones((1,jnp.shape(uf)[1]-1,1))) # Px by Q=(Qx by Qt)
    res = ftilde.reshape((jnp.shape(ftilde)[0],-1)) - out # F - Ls;
    return res