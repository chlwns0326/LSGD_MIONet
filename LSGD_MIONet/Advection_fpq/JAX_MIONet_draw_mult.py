...
def draw_sample(xy_in,uin_f,uin_g,uout,uout_pred,inds,keyword,savedir):
    ...
    for ind in inds:
        ...
        im = ax.plot(jnp.linspace(-1,1,jnp.shape(uin_g_ind)[0]), uin_g_ind)
        ax.set_title(f'Input function PQ')
...
# Directories (Current directory: /LSGD_MIONet/Advection_fpq)
folder = 'Models/'
keywords = ['Advection_fpq']
model_descs = ['L3W100']
phys = [1e-1]
lambdas = [1e-6]
...
for keyword,model_desc,phy,lamb,etime in zip(keywords,model_descs,phys,lambdas,etimes):
    ...
    if keyword == 'Advection_fpq':
        u_f_in_foo = jnp.zeros((1,Nx+1)) # 1D input
        u_g_in_foo = jnp.zeros((1,2*Nx+1)) # Long 1D input
        dataname = 'Advection_f_PQ_data'
        model_settings = [([100]*2,silu,1,False),([100]*2,silu,1,False),([100]*3,silu,1,False)]
    ...
    j = 1 # jump/stride
    uin_f_train = jnp.asarray(data['input_f_train'].astype('float64'))[:N_train,:][:,::j]
    uin_f_val = jnp.asarray(data['input_f_val'].astype('float64'))[:N_test,:][:,::j]
