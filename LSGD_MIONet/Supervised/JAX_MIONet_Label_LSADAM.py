...
LS_frac = 0 # if > 1, apply ALS step inside Adam epochs. 
weights_init = [1,1e-8]  # weights for data, regularization term, resp. 
...
if LS_num > 0: # LS+Adam
    ...
    total_WU = 8000 // adam_num 
    delay_init = delay_init_pre//adam_num
    model_dir = folder + keyword + '_ALSAdam_L' + str(LS_num) +'R' + str(adam_num) + \
        '_' + str(delay_init_pre) + '_HHH_SSS_bat' + str(bs) + 'SQto' + str(bn) + 'SQ_' + \
...
u_f_in_foo = jnp.zeros((batch_size[0],Nx+1)) # 1D input
u_g_in_foo = jnp.zeros((batch_size[1],Nx+1)) # 1D input
...
j = 1 # jump/stride
uin_f_train = jnp.asarray(data['input_train_f'].astype('float64'))[:N_train[0],:][:,::j]
uin_f_val = jnp.asarray(data['input_val_f'].astype('float64'))[:N_val,:][:,::j]
uin_g_train = jnp.asarray(data['input_train_D'].astype('float64'))[:N_train[1],:][:,::j]
uin_g_val = jnp.asarray(data['input_val_D'].astype('float64'))[:N_val,::j]
uout_train = jnp.asarray(data['output_train'].astype('float64'))[:N_train[0],:N_train[1],:,:][:,:,::j,:][:,:,:,::j]
...
# xy_phys = jnp.reshape(xy_in[1:,1:,:],(-1,2))
# xy_data = jnp.concatenate((xy_in[-1:-Ny-1:-1,0,:],xy_in[0,:,:]),axis=0) # (32+33 by 2)
...
train(params=params, optimizer=optimizer, seed=seedB, delay_init=delay_init,
      ...
      batch_size=batch_size, batch_new=batch_new, adam_num=adam_num, LS_num=LS_num, LS_frac=LS_frac,
